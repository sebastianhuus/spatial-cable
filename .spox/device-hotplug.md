...
- [ ] When the currently-targeted output device disconnects while relaying is active, the relay stops cleanly and the menu bar status text reflects that the device disconnected (no silent failure, no audio left trying to route into a dead device)
- [ ] When a new output device is connected, it appears in the output-device picker automatically — no manual "Refresh Devices" click required
- [ ] When a device disconnects while relaying is *not* active (idle, or a different device is selected), the output-device picker updates to drop it without user action
- [ ] The existing manual "Refresh Devices" button continues to work as a fallback/no-op if hotplug detection is already current
...
**Open question for next session**: should replugging the exact device that was previously selected (matched by stable UID, per `DeviceStore`'s existing persistence convention) auto-reselect it, or should that always require an explicit user pick? Not decided — resolve before or during implementation.
